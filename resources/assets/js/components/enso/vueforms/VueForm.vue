  ...
  <div class="form-wrapper">
    <form @submit.prevent="submit()">
      <div class="columns is-multiline">
        <div v-for="field in data.fields" class="column" :class="columnSize">
          <div class="field">
            <label class="label">
              {{ __(field.label) }}
...
export default {
  ...
  computed: {
    ...
    columnSize: () => 'is-' + parseInt(12 / this.data.columns)
  },
  methods: {
    create () {
    ...
    submit () {
      ...
      axios[this.data.method](this.getSubmitRoute(), this.getFormData())
        .then(response => {
          this.loading = false
          toastr.success(response.data.message)
          this.$emit('form-submit')

          if (this.data.method === 'post') this.$router.push({ name: response.data.redirect, params: { id: response.data.id } })
        })
        .catch(error => {
          this.loading = false
    ...
    destroy () {
      ...
      axios.delete(route(this.data.actions.destroy.route, this.data.actions.destroy.id, false))
        .then(response => {
          this.loading = false
          toastr.success(response.data.message)
