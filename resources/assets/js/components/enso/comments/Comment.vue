<template>
  <article class="media">
    ...
    <div class="media-right" v-if="!isNew && !isEditing">
      <div class="level-item" v-if="!isNew && !isEditing">
...
export default {
  ...
  computed: {
    ...
    isEditing: () => this.originalBody !== null
  },
  ...
  methods: {
    post () {
      ...
      axios.post(route('core.comments.store', [], false), this.postParams())
        .then(response => {
    ...
    update () {
      ...
      axios.patch(route('core.comments.update', this.comment.id, false), this.comment)
        .then(response => {
          this.$parent.$parent.loading = false
    ...
    syncTaggedUsers () {
      ...
      this.comment.taggedUserList.forEach((user, index) => {
        if (!this.comment.body.includes(user.fullName)) {
          this.comment.taggedUserList.splice(index, 1)
        }
      })
    },
    destroy () {
      ...
      axios.delete(route('core.comments.destroy', this.comment.id, false))
        .then(response => {
          this.$parent.$parent.loading = false
    ...
    getAvatarLink: (comment) => this.isNew
      ? this.avatarLink
