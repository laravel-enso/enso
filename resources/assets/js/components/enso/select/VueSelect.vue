<template>
  <div class="vue-select">
    <multiselect :value="value"
      ...
      <template slot="clear" slot-scope="props" v-if="!disabled">
...
export default {
  ...
  computed: {
    ...
    isServerSide: () => this.source !== null,
    hasSelection: () => (this.multiple && this.value.length) || (!this.multiple && this.value !== null),
    optionKeys: () => this.keyMap === 'number'
      ? Object.keys(this.optionList).map(Number)
      : Object.keys(this.optionList),
    matchedValue: () => {
      if (!this.multiple) return this.optionKeys.filter(option => option === this.value).length > 0

      return this.optionKeys.filter(option => this.value.filter(val => (val === option)).length > 0).length > 0
    }
  },
  filters: {
  ...
  methods: {
    getOptions () {
      ...
      axios.get(route(this.source, [], null), {params: this.getParams()})
        .then(response => {
    ...
    processOptions (response) {
      ...
      if (!this.query && !this.matchedValue) this.clear()
    },
    customLabel: (option) => this.optionList[option],
    update () {
      this.$nextTick(() => {
  ...
  mounted () {
    if (this.isServerSide) this.getOptions()
